...
<head>
    ...
    <title>ข้อมูล การเช่าของลูกค้า</title>
...
<body>
    ...
    <div class="texts">ข้อมูล การเช่าของลูกค้า</div>
    <!-- <div class="scrolltable"> -->
    <?php
    $stmt = $pdo->prepare("SELECT * from การเช่า");
    $stmt->execute();
    $num_produce=0;
        while ($row = $stmt->fetch()){
            $num_produce++;
    ...
    <table class="center">
        <tr>
            ...
            <th>รหัส Admin</th>
            <th>จำนวนที่เช่า</th>
            <th>วันเวลาที่เช่า</th>
        </tr>

        <?php
    $stmt = $pdo->prepare("SELECT * from การเช่า order by วันเวลาที่เช่า desc limit $offset,$record_show");
    $stmt->execute();
    while($row = $stmt->fetch()) : ?>
        <tr>
            <th><?=$row["รหัส_CD_DVD"]?></th>
            <th><?=$row["เลขที่ใบเสร็จ"]?></th>
            <th><?=$row["รหัส_สมาชิก"]?></th>
            <th><?=$row["รหัส_Admin"]?></th>
            <th><?=$row["จำนวนที่เช่า"]?></th>
            <th><?=$row["วันเวลาที่เช่า"]?></th>
        </tr>
        <?php endwhile ?>
    </table>
    <!-- </div> -->
