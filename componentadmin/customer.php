...
<body>
    ...
    <div class="texts">ข้อมูล ลูกค้า</div>
    <!-- <div class="scrolltable"> -->
    <?php
    $stmt = $pdo->prepare("SELECT * from สมาชิก");
    $stmt->execute();
    $num_produce=0;
        while ($row = $stmt->fetch()){
            $num_produce++;
    ...
    <table class="center">
        <tr>
            <th>รหัส สมาชิก</th>
            <th>Username สมาชิก</th>
            <th>Password สมาชิก</th>
            <th>ชื่อ-สกุล สมาชิก</th>
            <th>ที่อยู่ สมาชิก</th>
            <th>email</th>
            <th>เบอร์โทร สมาชิก</th>
        </tr>

        <?php
    $stmt = $pdo->prepare("SELECT * from สมาชิก limit $offset,$record_show");
    $stmt->execute();
    while($row = $stmt->fetch()) : ?>
        <tr>
            <th><?=$row["รหัส_สมาชิก"]?></th>
            <th><?=$row["Username_สมาชิก"]?></th>
            <th><?=$row["Password_สมาชิก"]?></th>
            <th><?=$row["ชื่อ_สกุล_สมาชิก"]?></th>
            <th><?=$row["ที่อยู่_สมาชิก"]?></th>
            <th><?=$row["email"]?></th>
            <th><?=$row["เบอร์โทร_สมาชิก"]?></th>
        </tr>
        <?php endwhile ?>
    </table>
    <!-- </div> -->
