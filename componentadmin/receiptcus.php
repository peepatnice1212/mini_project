...
<head>
    ...
    <title>ข้อมูล ใบเสร็จของลูกค้า</title>
...
<body>
    ...
    <div class="texts">ข้อมูล ใบเสร็จของลูกค้า</div>
    <!-- <div class="scrolltable"> -->
    <?php
    $stmt = $pdo->prepare("SELECT * from ใบเสร็จ");
    $stmt->execute();
    $num_produce=0;
        while ($row = $stmt->fetch()){
            $num_produce++;
    ...
    <table class="center">
        <tr>
            ...
            <th>จำนวนทั้งหมดที่ซื้อ</th>
            <th>รหัส สมาชิก</th>
        </tr>

        <?php
    $stmt = $pdo->prepare("SELECT * from ใบเสร็จ limit $offset,$record_show");
    $stmt->execute();
    while($row = $stmt->fetch()) : ?>
        <tr>
            <th><?=$row["เลขที่ใบเสร็จ"]?></th>
            <th><?=$row["ยอดชำระทั้งหมด"]?></th>
            <th><?=$row["จำนวนที่ซื้อ"]?></th>
